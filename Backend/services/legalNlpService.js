/**
 * Legislative NLP & Statutory Citation Engine ("The Sword of Damocles")
 * Maps civic issue categories to Indian Legal Statutes, IPC Sections, and Constitutional Articles.
 */
...
export const getStatutoryCitations = (category) => {
    switch (category) {
        ...
        case 'Duplicate Beneficiary / Welfare Fraud':
            return {
                primaryAct: "Prevention of Corruption Act 1988 — Section 13(1)(d)",
                constitutionalArticle: "Article 14 of the Constitution of India (Equality Before Law & Non-Arbitrary Distribution of Welfare)",
                ipcSections: ["Section 420 IPC (Cheating & Dishonest Inducement)", "Section 468 IPC (Forgery for Purpose of Cheating)"],
                municipalAct: "Aadhaar (Targeted Delivery of Financial & Other Subsidies) Act 2016 — Section 40",
                precedents: [
                    "K.S. Puttaswamy v. Union of India (2018) 1 SCC 1 — Welfare delivery auditing and leak prevention.",
                    "State of UP v. Manohar (2005) 2 SCC 788 — Strict accountability in public fund disbursement."
                ],
                legalNoticeTitle: "STATUTORY DEMAND NOTICE FOR WELFARE BENEFICIARY FRAUD AUDIT",
                mandamusWritGround: "Arbitrary and corrupt siphoning of public welfare funds violating Article 14 equal access protections."
            };
...
export const generatePilDraftData = (report, upvoteCount = 52) => {
    ...
    return {
        ...
        subject: `PUBLIC INTEREST LITIGATION (PIL) FOR ISSUANCE OF A WRIT OF MANDAMUS DIRECTING IMMEDIATE REPAIR & STATUTORY SANCTIONS FOR REPORTED CIVIC HAZARD: "${report.title.toUpperCase()}"`,
        geoCoordinates: `${report.latitude?.toFixed(4) || '12.9716'}, ${report.longitude?.toFixed(4) || '77.5946'}`,
        photoProofUrl: report.imageUrl,
        citations,
        factsSummary: `The Petitioner Collective brings to the immediate attention of this Hon'ble Court a severe, unaddressed civic hazard located at ${city} (Geo-Location: ${report.latitude?.toFixed(4)}, ${report.longitude?.toFixed(4)}). Despite multiple digital audit submissions and ${upvoteCount} verified citizen upvotes on the JanDarpan Public Transparency Portal, the Respondents have exhibited gross administrative inertia, violating statutory obligations under ${citations.primaryAct}.`,
        legalGrounds: [
        ...
        prayer: [
            `a) Issue a Writ of Mandamus directing Respondents No. 1 to 3 to initiate physical repair and rectification work on "${report.title}" within 72 hours;`,
